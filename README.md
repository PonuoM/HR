...
- ✅ รัน `schema.sql` (ผ่าน phpMyAdmin หรือ terminal)
- ✅ โฟลเดอร์ `uploads/` ต้อง writable (`chmod 755`)

### 4. รัน Migrations บน Production (ผ่าน URL)

หลังจาก upload ขึ้น server แล้ว รัน migrations ผ่านเบราว์เซอร์:

```
https://hr.prima49.com/database/run_migration.php?key=prima49migrate
```

> ⚠️ **ต้องใส่ `?key=prima49migrate`** ไม่งั้นจะโดนบล็อก (403)

จะแสดงผลลัพธ์:
- ✅ สำเร็จ — statement ที่รันผ่าน
- ⏭️ ข้าม — ถ้ามี table/column อยู่แล้ว (รันซ้ำได้ไม่พัง)
- ❌ ล้มเหลว — ถ้ามี error จริง
